...
Option parse_option(std::vector<std::string> const &args)
{
    ...
    opt.Va_path = args[i++]; //test文件在前
    opt.Tr_path = args[i++]; //train文件在后

    return opt;
}

/*
能不能描述一下init_model的大概内容?
*/
void init_model(Model &model)
{
    ...
    float * w = model.W.data(); //指向vector中第一个数据的地址
...
int main(int const argc, char const * const * const argv)
{
    ...
    std::cout << "done\n" << std::flush;
    std::cout << "tr nr_feature: " << Tr.nr_feature << std::endl;
    std::cout << "tr nr_instance: " << Tr.nr_instance << std::endl;
    std::cout << "tr nr_field: " << Tr.nr_field << std::endl;

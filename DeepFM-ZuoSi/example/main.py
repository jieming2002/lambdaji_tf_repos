...
from DataExtend import add_numeric
...
def _load_avazu_ctr_data():
    _print("load data begin")
    add_numeric(config.TRAIN_FILE, config.TRAIN_EXT_FILE)
    add_numeric(config.TEST_FILE, config.TEST_EXT_FILE, False)

    dfTrain = pd.read_csv(config.TRAIN_EXT_FILE, dtype=config.EXT_DTYPES) 
    dfTest = pd.read_csv(config.TEST_EXT_FILE, dtype=config.EXT_DTYPES)
...
# 后面这两个暂不执行了,因为DataParse里面dfi没有做内存copy,dfTrain已经发生了变动
"""
# ------------------ FM Model ------------------

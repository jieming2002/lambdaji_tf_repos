...
class DeepFM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default(): #在self.graph这个图下操作
            ...
            # 注意这里W和X用的是点乘,所以还要再做一个加和,参数2的目的只是实现了降维
            ...
            self.summed_features_emb = tf.reduce_sum(self.embeddings, 1)  # None * K,注意这连续几个None * K
            ...
            # exponential decay, add by zuosi
            self.global_step = tf.Variable(0, trainable=False)
            self.decay_steps = self.n_samples/self.batch_size
            _print("decay_steps: %d" % self.decay_steps)
            
            self.decay_learning_rate = tf.train.exponential_decay(
                    self.learning_rate,             #初始学习率
                    self.global_step,               #步数
                    self.decay_steps,               #衰减步长
                    0.90                            #衰减系数
            )
            ...
            # number of params,计算总的参数个数
            total_parameters = 0
            for variable in self.weights.values():
                # variable为tf的Vairable所以才有get_shape
            ...
            if self.verbose > 0:
                _print("#params: %d" % total_parameters)
    ...
    def fit(self, Xi_train, Xv_train, y_train,
        ...
        for epoch in range(self.epoch):
            ...
            for i in range(total_batch):
                ...
                step += 1
                if step%20 == 0:
                    _print("After %d training step(s), learning_rate is %g, loss on training batch is %g, [%.1f s]" % \
                        (step, self.sess.run(self.decay_learning_rate), loss, time() - t2))
            ...
            if self.verbose > 0 and epoch % self.verbose == 0:
                if has_valid:
                    line = "[%d] train-result=%.4f, valid-result=%.4f [%.1f s]" % (epoch + 1, train_result, valid_result, time() - t1)

                else:
                    line = "[%d] train-result=%.4f [%.1f s]" % (epoch + 1, train_result, time() - t1)
                _print(line)
